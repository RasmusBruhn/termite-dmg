...
mod tests {
    ...
    fn test() {
        let model = DataModel {
            data_types: Vec::new(),
            footers: HashMap::from([("A".to_string(), "a".to_string()), ("B".to_string(), "b".to_string())]),
            headers: HashMap::from([("A".to_string(), "a".to_string())]),
        };

        println!("{}", model.export_yaml().unwrap());
        println!("{}", model.export_json().unwrap());
    }
}
