...
to strings with each object in the map referering to the string to add as
header/footer for the file refered to by the key. Currently the only supported
keys are "cpp-header", "cpp-source", and "dart" for the generated ".h", ".cpp",
and ".dart" files.
...
A data type is a map from the name of the type (a string) to the data for the
type. This must include type specific "data" and optionally a "description". The
type specific data defines how the type is implemented and may be types like
structs, enums or arrays.
