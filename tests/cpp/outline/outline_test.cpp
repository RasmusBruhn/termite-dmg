...
std::optional<std::string> test_from_map_1() {
  ...
  if (!value_read_correct.is_ok()) {
    std::stringstream ss;
    ss << "Unable to construct DataType1 from a map: "
...
std::optional<std::string> test_from_map_2() {
  ...
  if (!value_read_correct.is_ok()) {
    std::stringstream ss;
    ss << "Unable to construct DataType2 from a map: "
       << value_read_correct.get_err();
    return ss.str();
  }
  return std::nullopt;
...
std::optional<std::string> test_from_map_large_1() {
  termite::Node node_many = termite::map{{"field1", "1"}};
  auto value_read_many = node_many.to_value<test::DataType1>();
  if (!value_read_many.is_ok()) {
    std::stringstream ss;
    ss << "Unable to construct DataType1 from a map with too many fields: "
...
std::optional<std::string> test_from_map_large_2() {
  ...
  if (!value_read_many.is_ok()) {
    std::stringstream ss;
    ss << "Unable to construct DataType2 from a map with too many fields: "
       << value_read_many.get_err();
    return ss.str();
  }
  return std::nullopt;
...
int main() {
  ...
  auto functions = {
      test_default_1,         test_default_2,         test_from_map_1,
      test_from_map_2,        test_from_map_large_1,  test_from_map_large_2,
      test_error_node_type_1, test_error_node_type_2,
  };
