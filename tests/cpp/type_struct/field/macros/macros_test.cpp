...
std::optional<std::string> test_eq_self() {
  ...
  auto value2 = test::DataType({{"field1", (termite::integer)-2}, {"field2", 3.5}});
  if (value2 != value2) {
    return "A struct was not equal to itself";
  }
  return std::nullopt;
...
std::optional<std::string> test_load() {
  auto value = test::DataType({{"field1", (termite::integer)-2}, {"field2", 3.5}});
...
std::optional<std::string> test_load_extra_fields() {
  auto value = test::DataType({});
  termite::Node node_many = termite::map{{"field3", "test"}};
