#include "generated/max_length_array.h"
...
std::optional<std::string> test_load() {
  test::DataType value =
      test::InnerType({(termite::integer)1, (termite::integer)2});
  termite::Node node = termite::list{"1", "2"};
...
std::optional<std::string> test_error() {
  termite::Node node = termite::list{"1", "2", "3"};
  auto value_read = node.to_value<test::DataType>();
  if (value_read.is_ok()) {
    std::stringstream ss;
