...
String? testFromString() {
  final result = termite_json.fromString('{"field1":"Test1","field2":["Test2","Test3"]}');
  ...
  if (node.map['field1'] is! termite.Value || (node.map['field1'] as termite.Value).value != 'Test1') {
...
String? testToStringAndBack() {
  final node = termite.Node.mapping({
    ...
    'field2': termite.Node.sequence([termite.Node.value('Test2'), termite.Node.value('Test3')]),
  });
...
String? testFileRoundtrip() {
  final file = File('json_test_runtime.json');
  file.writeAsStringSync('{"field1":"Test1","field2":["Test2","Test3"]}');

  final fileData = File('json_test.json').readAsStringSync();
  ...
  if (reparsed.value.toObject().toString() != parsed.value.toObject().toString()) {
